...
export default function Feed() {
	...
	async function loadFeed() {
		try {
			setLoading(true);
			const { data } = await api.get('/owner/feed');
			setPosts(data.data || []);
		} catch (error) {
	...
	async function likePost(postId) {
		try {
			const { data } = await api.post(`/owner/posts/${postId}/like`);
	...
	async function deletePost(postId) {
		...
		try {
			await api.delete(`/owner/posts/${postId}`);
	...
	async function sharePost(e) {
		...
		setSharing(true);
		try {
			const { data } = await api.post(`/owner/posts/${selectedPost.id}/share`, {
						...
						<div className="space-y-4">
							...
							<div className="bg-white rounded-lg shadow p-6">
								<div className="flex items-center justify-between">
									<div>
										<h1 className="text-2xl font-semibold text-gray-900">Gym Owner Feed</h1>
										<p className="text-gray-600 mt-1">Connect with your gym members and share updates</p>
									</div>
