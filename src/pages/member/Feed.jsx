...
export default function Feed() {
	...
	async function loadFeed() {
		try {
			...
			console.log('Posts in feed:', data.data);
			setPosts(data.data || []);
		} catch (error) {
	...
	async function addComment(e, postId) {
		...
		try {
			await api.post(`/member/posts/${postId}/comments`, {
									...
									<div key={post.id} className="bg-white rounded-lg shadow">
										...
										{post.shares && post.shares.length > 0 && (
											<div className="mx-4 mb-3 border border-gray-200 rounded-lg bg-gray-50">
												{/* Debug Info */}
												{console.log('Post shares debug:', {
													postId: post.id,
													postContent: post.content,
													shares: post.shares,
													firstShare: post.shares[0],
													originalPost: post.shares[0]?.originalPost
												})}
												
												{post.shares[0].originalPost ? (
													<>
													...
													<div className="p-3 text-sm text-gray-500">
														Debug: No original post data found
													</div>
												)}
											</div>
										)}
